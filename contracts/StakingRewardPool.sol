...
contract StakingRewardPool is StakingPool  {
    ...
    function newRewardPeriod(uint reward, uint from, uint to) public  {
    ...
    function deleteRewardPeriod(uint index) public  {
    ...
    function depositReward(uint amount) internal  {
        rewardToken.transferFrom(msg.sender, address(this), amount);
    }


    function startStake(uint amount, bool isLocked, uint duration ) public override {

        uint periodId = getCurrentRewardPeriodId();
        require(periodId > 0, "No active reward period found");
        update();

        super.startStake(amount, isLocked, duration);
